...
export const generateTasks = (info: any): ThunkAction<void, RootState, unknown, AnyAction> => async (dispatch) => {
    ...
    messages.push({role:"user", content:`Format the times as a list of json objects where event_id field is a unique id for the event, title field is the name of the event, start field is the start of the event, end field is the end of the event and uid field is ${info.uid}.`})
